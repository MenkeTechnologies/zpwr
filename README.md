...
## Diagnostics
The test suite contains thousands of [zunit](https://github.com/zunit-zsh/zunit) tests across `tests/*.zsh` covering autoload function existence, type resolution, whence/which lookups, function body validation, fpath integrity, source syntax, script readability, shebang detection, verb callability, environment variable isolation, and behavioral checks for `scripts/lib.sh` helpers. Primary files for `lib.sh` coverage are `tests/t-lib-fns.zsh`, `tests/t-lib-extended.zsh`, `tests/t-lib-commands.zsh`, and `tests/t-lib-more.zsh` (logging, `zpwrLogColor` / `zpwrLogConsoleHeader`, `zpwrWizard` help, broad `zpwrCommandExists` checks, verb map spot checks, `scripts/init.sh`, `boxPrint.pl`, and related helpers). GitHub Actions runs the same `zunit --verbose tests/*.zsh` command as local runs.

CI workflow (`.github/workflows/ci.yml`): Ubuntu `ubuntu-latest`, installs dependencies with `--no-install-recommends`, shallow-clones [MenkeTechnologies/zunit](https://github.com/MenkeTechnologies/zunit) on cache miss, caches the built `zunit` binary under `~/.zpwr/.cache/zunit-bin` (key includes a hash of the workflow file plus `restore-keys` fallbacks), verifies `zunit` is executable and prints `--version` before the suite, sets `TERM`/`COLUMNS`/`LINES`, creates the same home-directory stubs the tests expect, and uses `concurrency` so a new push cancels an in-progress run on the same ref (avoids piling up duplicate 30+ minute jobs). The test job timeout is 120 minutes.
