...
## Diagnostics
The test suite contains thousands of [zunit](https://github.com/zunit-zsh/zunit) tests across `tests/*.zsh` covering autoload function existence, type resolution, whence/which lookups, function body validation, fpath integrity, source syntax, script readability, shebang detection, verb callability, environment variable isolation, and behavioral checks for `scripts/lib.sh` helpers. Primary files for `lib.sh` coverage are `tests/t-lib-fns.zsh`, `tests/t-lib-extended.zsh`, `tests/t-lib-commands.zsh`, `tests/t-lib-more.zsh`, `tests/t-lib-batch2.zsh`, `tests/t-lib-batch3.zsh`, `tests/t-lib-batch4.zsh`, `tests/t-lib-batch5.zsh`, `tests/t-lib-batch6.zsh`, `tests/t-lib-batch7.zsh`, `tests/t-lib-batch8.zsh`, and `tests/t-lib-batch9.zsh` (logging, `zpwrLogColor` / `zpwrLogConsoleHeader`, `zpwrWizard` help, broad `zpwrCommandExists` checks, `ZPWR_VERBS` registration and size vs `env/zpwr.zsh`, **`bash -n`** / **`zsh -n`** on env and script entrypoints (`scripts/lib.sh`, `env/zpwr.zsh`, `env/grc.zsh`, `env/.p10k.zsh`, `scripts/init.sh`), **`perl -c`** on all `scripts/*.pl`, **`python3 -m py_compile`** on `scripts/*.py`, **`bash -n`** on `install/*.sh` and on all top-level `scripts/*.sh` plus `scripts/macOnly/*.sh`, `perl -c` on `boxPrint.pl` (also in batch4), verb map spot checks, **`ci.yml`** regression greps (cache `hashFiles` path, `DEBIAN_FRONTEND`, `netcat-openbsd`, checkout `path: zpwr`, `permissions`, `curl` retries, etc.), `scripts/init.sh`, workflow metadata greps, and related helpers). GitHub Actions runs the same `zunit --verbose tests/*.zsh` command as local runs.
