...
if !empty(glob("$ZPWR_FZF_DIR/plugin/fzf.vim"))
    source $ZPWR_FZF_DIR/plugin/fzf.vim
endif
...
function! WriteToNVimInfo()
    ...
    if empty(dir)
        if isdirectory(expand('~/.zpwr/local'))
            let dir=expand('~/.zpwr/local')
        else
            return 0
        endif
    endi

    let viminfo = dir.'/.nviminfo'
