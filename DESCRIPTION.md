...
Above this foundation sits the **verb system**, the primary interface through which users interact with ZPWR's capabilities. Over 410 subcommands are registered in a central associative array (`ZPWR_VERBS`), each mapping a verb name to a function and a description. Typing `zpwr` followed by a tab presents a colorized menu of every available command, complete with grouped descriptions and neon-styled separators. This design prioritizes discoverability — a user need not memorize commands but can browse and search them interactively, aided by fzf-powered fuzzy finding via `zpwr verbs`.
