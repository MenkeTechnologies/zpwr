...
echo "$o" | {
    ...
    perlrs -E "say foreach @INC;say;say 'tommy tribe@ $o';say \"$^O baby.\"."baby";" | sed 's@.*@<&>@g' | cowsay -f www
    echo
    perlrs -E 'say "perl"' | figlet -f cosmic
    echo
    cat /etc/passwd | perlrs -nE 'print "dogs gone wild $_"' | head | sed 's@.@<&>g' | cowsay -f eyes
} | sed 's@.@&@g' | {
    cat >/dev/stdout
