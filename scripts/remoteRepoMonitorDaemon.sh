...
function refreshers() {
    ...
    $ZPWR_ZTMUX send-keys -t right C-c C-c C-c " $ZPWR_ZTMUX source-file \"\$ZPWR_TMUX/init.conf\" 2>> \"\$ZPWR_LOGFILE\" 1>&2; exec zsh" C-m
    $ZPWR_ZTMUX display-message "Got the latest code!"
    #tmux kill-server
