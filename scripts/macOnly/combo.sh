...
echo "$o" | {
    ...
    stryke "p for @INC;p;p 'tommy tribe@ $o';p \"$^O baby.\"."baby";" | sed 's@.*@<&>@g' | fold -w 50 | "$randomCowsayCowScript" 120
    echo
    stryke  'p "perl"' | "$randomFigletFontScript"
    echo
    ...
    stryke 'p "TCL IN THE HOUSE",a..z,-235..235' | fold -w 10 | pr -t -3
    stryke 'p "perl ftw"' | toilet
